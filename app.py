...
with st.sidebar:
    ...
    # 2. Pipeline is NOT running
    if not st.session_state.get("documents_decision_made") and not st.session_state.get("pipeline_running"):
        if st.session_state.get("just_uploaded_repo") or (selected_repo and not uploaded_repo_name):
            # User just uploaded a repo OR selected from dropdown - ask if they want to add documents
            st.divider()
            st.subheader("📎 Add Supporting Documents?")
            st.caption("📌 Documents will be **analyzed** and **incorporated** into the BRD generation.")
