# Agent 6: Acceptance Criteria - PowerShell Orchestrator
...
Write-Host "Agent 6: Acceptance Criteria" -ForegroundColor Cyan
Write-Host "==================================" -ForegroundColor Cyan
Write-Host ""
...
$pythonScript = Join-Path $PSScriptRoot "agent_6_acceptance_criteria.py"
...
Write-Host "Executing Agent 6..." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Write-Host ""
    if ($exitCode -eq 0) {
        Write-Host "Agent 6 completed in $($duration.ToString('F2')) seconds" -ForegroundColor Green
        exit 0
    } else {
        Write-Error "Agent 6 failed with exit code: $exitCode"
        exit $exitCode
    }
} catch {
    Write-Error "Agent 6 execution error: $_"
    exit 1
}
