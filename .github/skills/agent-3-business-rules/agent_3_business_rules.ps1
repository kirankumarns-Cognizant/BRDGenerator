# Agent 3: Business Rules - PowerShell Orchestrator
...
Write-Host "Agent 3: Business Rules" -ForegroundColor Cyan
Write-Host "==================================" -ForegroundColor Cyan
Write-Host ""
...
$pythonScript = Join-Path $PSScriptRoot "agent_3_business_rules.py"
...
Write-Host "Executing Agent 3..." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Write-Host ""
    if ($exitCode -eq 0) {
        Write-Host "Agent 3 completed in $($duration.ToString('F2')) seconds" -ForegroundColor Green
        exit 0
    } else {
        Write-Error "Agent 3 failed with exit code: $exitCode"
        exit $exitCode
    }
} catch {
    Write-Error "Agent 3 execution error: $_"
    exit 1
}
